...
This script quiries an Atlassian Jira server for users and the groups they are members of. Specific or all users can be requested. User group membership is written to standard output and can be redirected to a file for later use as base of comparison or a state to revert to.

I created this script due to Jira losing group membership sporadically for users authenticated by LDAP (group membership was not stored in LDAP).
...
SERVER_URL = 'https://jira.server.com'
...
def get(params = dict(), api = '/rest/api/2/'):
    url = api
    if params:
...
def getuser(username):
    ...
    ret = get({ 'username': username, 'expand': 'groups.items,applicationRoles.items'}, '/rest/api/2/user')
...
def getallusers():
    ...
    ret = get({ 'username': '.', 'startAt': 0, 'maxResults': 1000 }, '/rest/api/2/user/search')
    if ret[0]:
        ulist = list()
        for user in ret[1]: 
...
def getallusergroups():
    ...
    for user in allusers:
        ...
        utmp = getuser(user)
        grouplist = list() 
        for group in utmp['groups']['items']:
...
def addusergroup(usergroup, force = False):
    if args.debug: print "Addusergroup - user: {} group: {}".format(usergroup[0], usergroup[1])
    if not force:
        # check if user belongs to the requested group
        utmp = getuser(usergroup[0])
        for group in utmp['groups']['items']:
            if group['name'] == usergroup[1]:
                print "User already belongs to group, not modifying membership"
                return
    ret = post({ 'groupname': usergroup[1] }, { 'name': usergroup[0] }, '/rest/api/2/group/user')
    if ret[0]:
